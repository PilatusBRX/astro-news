...
function calculateSimilarityScore(
  ...
  let categoryScore = 0;
  if (categoryWeight > 0 && currentPost.data.categories && targetPost.data.categories) {
    const currentCategories = new Set(currentPost.data.categories.map(c => c.toLowerCase()));
    const targetCategories = new Set(targetPost.data.categories.map(c => c.toLowerCase()));
    categoryScore = [...currentCategories].some(c => targetCategories.has(c)) 
      ? categoryWeight 
      : 0;
  }
